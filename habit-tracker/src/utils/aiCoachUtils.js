...
export const getQuickPrompt = (type, habits, moodLog) => {
  switch (type) {
    ...
    case 'focus_today': {
      ...
      const pendingHabits = habits.filter(h => !h.completionLog?.[Object.keys(h.completionLog || {})[0]]).slice(0, 3);
      const habitNames = pendingHabits.map(h => (h.icon || '⭐') + ' ' + h.name).join(', ');
      return `Based on my habits: ${habitNames || 'none specific'}, which ones should I focus on today and why? Keep it brief.`;
    }
    default:
