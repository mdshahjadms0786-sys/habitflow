...
export const addFocusSession = (session) => {
  const sessions = getFocusSessions();
  sessions.unshift({ ...session, completedAt: new Date().toISOString() });
  localStorage.setItem(STORAGE_KEY, JSON.stringify(sessions.slice(0, 500)));
  addPoints('focusSessionCompleted');
  return sessions;
};
