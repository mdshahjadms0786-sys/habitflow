...
export function getLeagueRank(competitors, _userId = 'user') {
...
export function updateWeeklyProgress(habits) {
  ...
  if (data.weekStart !== getWeekStart()) {
    return initializeLeague(habits);
  }

  // Update competitor points deterministically instead of regenerating
  const pointsDiff = Math.max(0, weeklyPoints - data.weeklyPoints);
  
  let newCompetitors = data.competitors.map(comp => {
    if (comp.isUser) {
      return { ...comp, weeklyPoints };
    }
    // Simulate competitor progress: they gain roughly similar points to user, with some variation
    if (pointsDiff > 0) {
      const competitorGain = Math.floor(pointsDiff * (0.5 + Math.random()));
      return { ...comp, weeklyPoints: comp.weeklyPoints + competitorGain };
    }
    return comp;
  });

  newCompetitors.sort((a, b) => b.weeklyPoints - a.weeklyPoints);
  newCompetitors = newCompetitors.map((c, i) => ({ ...c, rank: i + 1 }));
