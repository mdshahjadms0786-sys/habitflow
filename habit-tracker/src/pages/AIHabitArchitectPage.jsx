...
function AIHabitArchitectPage() {
  ...
  useEffect(() => {
    if (user && hasFeature('aiHabitArchitect')) {
      fetchEliteFeatureData('aiArchitectPlan', user.id).then(data => {
        if (data && data.plan) {
  ...
  const generatePlan = () => {
    ...
    setTimeout(async () => {
      ...
      if (user) {
        await saveEliteFeatureData('aiArchitectPlan', {
          goal: selectedGoal,
          plan,
          applied: false,
  ...
  const handleStartOver = async () => {
    if (user) {
      await saveEliteFeatureData('aiArchitectPlan', {}, user.id);
    }
        ...
        {step === 3 && (
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-secondary)', marginBottom: '24px' }}>
              Your 90-day transformation journey is currently active. Your habits have been added to your dashboard.
            </p>
            <button
