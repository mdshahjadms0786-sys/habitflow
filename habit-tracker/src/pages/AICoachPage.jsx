...
Stack habits with existing routines for better adherence. 🔥`;
  ...
  if (message.includes('week') || message.includes('analyz') || message.includes('focus on today')) {
...
const DailyInsightCard = ({ habits, moodLog }) => {
...
Give me one personalized tip for today based on my habit data. Keep it concise (max 50 words).`;
      ...
      const fallback = completionPct > 70
        ? "Great momentum! Keep it up today. You're on fire! 🔥"
        : completionPct > 40
          ? "Solid progress. Focus on your weakest habit to improve."
          : "Fresh start today. Pick your easiest habit first!";
      setInsight(fallback);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Ignore invalid cached insight data.
      }
    }
    generateInsight();
  }, [habits, moodLog]);
...
const ChatInterface = ({ habits, moodLog }) => {
  const [messages, setMessages] = useState(() => {
    try {
      const saved = localStorage.getItem('ht_ai_chat_messages');
      if (saved) {
  ...
  return (
    <div style={{ backgroundColor: 'var(--surface)', borderRadius: '16px', padding: '20px', border: '1px solid var(--border)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
        ...
        <button
          onClick={() => {
            setMessages([{ role: 'assistant', content: "Hi! I'm your HabitFlow Coach 🤖 How can I help you with your habits today?", timestamp: new Date() }]);
            localStorage.removeItem('ht_ai_chat_messages');
          }}
          style={{
            padding: '4px 10px',
            borderRadius: '6px',
...
const AICoachPage = () => {
  ...
  return (
    <div style={{ padding: '24px', paddingBottom: '80px' }}>
      ...
      <DailyInsightCard habits={safeHabits} moodLog={moodLog || {}} />

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '24px', alignItems: 'start' }}>
        <ChatInterface habits={safeHabits} moodLog={moodLog || {}} />
