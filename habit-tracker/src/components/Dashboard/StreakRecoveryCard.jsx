...
import { getTokens, spendGraceToken, spendFreezeToken, spendRecoveryToken } from '../../utils/recoveryUtils';
...
const StreakRecoveryCard = ({ habits, onTokenUsed }) => {
  ...
  const handleUseGrace = (habitId) => {
    const success = spendGraceToken(habitId);
    if (success) {
      setTokens(getTokens());
  ...
  const handleUseFreeze = () => {
    const success = spendFreezeToken();
    if (success) {
      setTokens(getTokens());
  ...
  const handleUseRecovery = (habitId, missedDate) => {
    const success = spendRecoveryToken(habitId, missedDate);
    if (success) {
      setTokens(getTokens());
